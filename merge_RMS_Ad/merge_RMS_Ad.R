...
#for(k in 1:1){
  ...
  for(i in 1:length(RMS_filenames)){
    ...
    RMS_data[, Week:=format(week_end,format="%W/%Y")]
    RMS_data[, product_module_code:=strtoi(dir_name, base=10)]
    ...
    if(file.exists(ad_filename)){
      ...
      if(!associated_brands & nrow(Ad_data)>0){
        filter_brands = string_matches[brand_code_uc==RMS_brand_code & 
                                       product_module_code==strtoi(dir_name, base=10) & 
      ...
      if(nrow(Ad_data)>0){
        ...
        Ad_data[, WeekDate:=as.Date(paste0("1/",Week),format="%w/%W/%Y")]
        setkey(Ad_data, BrandCode, MarketCode, WeekDate)
        Ad_data = Ad_data[CJ(unique(BrandCode), unique(MarketCode), 
                             seq(min(WeekDate), max(WeekDate), by=7))]
    
        # Calculate Ad stock and Total values
        print("Calculating ad stock...")
        setorder(Ad_data, WeekDate)
        ...
        if(b){
          ...
          setcolorder(Ad_data, c("BrandCode", "MarketCode", "Week", "WeekDate",
                                 "National_GRP", "Local_GRP", "National_occ", "Local_occ",
                                 "National_spend", "Local_spend"))
        ...
        Ad_data[,`:=`(National_GRP = roll_sum(National_GRP, n = window+1, weights = geom_weights,
                      ...
                      Local_occ = roll_sum(Local_occ, n = window+1, weights = geom_weights,
                                           normalize = F, align = "right", fill = NA)),
                      by=c("BrandCode", "MarketCode")]
        Ad_data[, Total_GRP:=rowSums(.SD, na.rm=T), .SDcols=c("National_GRP", "Local_GRP")]
        Ad_data[, Total_occ:=rowSums(.SD, na.rm=T), .SDcols=c("National_occ", "Local_occ")]
        Ad_data[, Week:=format(WeekDate,format="%W/%Y")]
        Ad_data[, WeekDate:=NULL]
        ...
        output = merge(RMS_data, Ad_data, by.x=c("dma_code", "Week"),
                       by.y=c("MarketCode", "Week"), all.x=T) # Keep RMS even if there is no advertising
