...
#for(k in 1:1){
  ...
  # Read competitors data directly
  ...
  # Read brands in top 100 and add competitors
  if(length(merged_filenames)>1){
    for(i in 1:length(merged_filenames)){
      ...
      if(RMS_brand %in% top100_brandcodes){
        ...
        # dcast
        brand_data = dcast(brand_data, dma_code + store_code_uc + week_end + year ~ brand_type,
                           value.var = list("units", "price", "promo_percentage","Total_GRP","Total_occ"), 
                           fun=list(mean, mean, mean, sum, sum))
        brand_data[Total_GRP==0, Total_GRP:=NA]
        brand_data[Total_occ==0, Total_occ:=NA]
        brand_data[, units_mean_comp:=NULL]
        names(brand_data) = sapply(names(brand_data), function(x){gsub("_mean|_sum","",x)})
